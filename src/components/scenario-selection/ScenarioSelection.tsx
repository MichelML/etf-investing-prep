...
class ScenarioSelection extends React.Component<Partial<IETFState & {setValue: (...args: any[]) => void}>> {
  ...
  private setStartDate(value: string, ...args: any[]) {
    if (this.props.setValue) {this.props.setValue(Actions.setStartDate, parseInt(value, 10));}
  }

  private setEndDate(value: string, ...args: any[]) {
    if (this.props.setValue) {this.props.setValue(Actions.setEndDate, parseInt(value, 10));}
  }
  private setMonthlyInvestment(e: React.ChangeEvent<HTMLInputElement>) {
    if (this.props.setValue) {this.props.setValue(Actions.setMonthlyInvestment, e.target.value);}
  }
  private setRatioWeight(e: React.ChangeEvent<HTMLInputElement>) {
    if (this.props.setValue) {this.props.setValue(Actions.setRatioWeight, e.target.value);}
  }
  private setMonthlyTransactionCost(e: React.ChangeEvent<HTMLInputElement>) {
    if (this.props.setValue) {this.props.setValue(Actions.setMonthlyTransactionCost, e.target.value);}
  }
  private setManagerialCost(e: React.ChangeEvent<HTMLInputElement>) {
    if (this.props.setValue) {this.props.setValue(Actions.setManagerialCost, e.target.value);}
  }
}
